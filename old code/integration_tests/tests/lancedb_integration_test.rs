//! Integration test for LanceDB integration
//!
//! This test verifies that the InformationRetrievalSkill can write to and read from LanceDB.

use pandora_tools::skills::information_retrieval_skill::{Concept, InformationRetrievalSkill};
use std::collections::HashMap;
use tempfile::TempDir;

#[tokio::test]
async fn test_lancedb_integration() {
    ...
    skill
        .add_concept(concept1.clone())
        .await
        .expect("Failed to add concept1");
    skill
        .add_concept(concept2.clone())
        .await
    ...
    let results = skill
        .search_by_vector(&query_vector, 2)
        .await
    ...
    assert!(
        result_ids.contains(&"concept_1".to_string()),
        "Should contain concept_1"
    );
    assert!(
        result_ids.contains(&"concept_2".to_string()),
        "Should contain concept_2"
    );
    ...
    println!(
        "✅ LanceDB integration test passed! Successfully wrote and read concepts from LanceDB."
    );
}
