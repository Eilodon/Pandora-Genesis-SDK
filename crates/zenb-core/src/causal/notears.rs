...
impl Notears {
    ...
    pub fn fit(&self, data: &DMatrix<f32>) -> DMatrix<f32> {
        ...
        if n_samples < 10 {
            log::warn!(
                "NOTEARS: Very few samples ({}), results may be unreliable",
                n_samples
            );
        }
        ...
        log::info!(
            "NOTEARS: Starting optimization (n_vars={}, n_samples={})",
            n,
            n_samples
        );
    ...
    fn minimize_aug_lagrangian(
        ...
        for _ in 0..self.config.max_inner_iter {
            ...
            let h = self.acyclicity(&w);
            let grad =
                grad_loss + (alpha + rho * h) * grad_h + self.config.lambda * self.grad_l1(&w);
...
mod tests {
    ...
    fn test_notears_simple_chain() {
        ...
        // Check acyclicity
        assert!(
            notears.acyclicity(&w).abs() < 0.1,
            "Result should be acyclic"
        );
    }
