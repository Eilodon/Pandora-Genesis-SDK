...
fn test_scientist_wires_discovery_to_causal_graph() {
    ...
    for i in 0..iterations {
        ...
        if i % 10 == 0 {
            // println!("Tick {}: Scientist State = {:?}", i, eng.causal.scientist().state_name());
        }
    }
    ...
    let _effect = eng
        .causal
        .graph
        .get_effect(Variable::HeartRate, Variable::HeartRateVariability);
    ...
    if !has_learned_edges {
        println!("Scientist State: {:?}", eng.causal.scientist().state_name());
        println!("Crystallized: {:?}", eng.causal.scientist().crystallized);
        println!("Rejected: {:?}", eng.causal.scientist().rejected);
    }
    ...
    let hrv_to_hr = eng
        .causal
        .graph
        .get_effect(Variable::HeartRateVariability, Variable::HeartRate);

    assert!(
        hr_to_hrv > 0.0 || hrv_to_hr > 0.0,
        "Should detect relationship between HR and HRV"
    );
}
