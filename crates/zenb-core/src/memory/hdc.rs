...
impl HdcMemory {
    ...
    pub fn stats(&self) -> (usize, u64, u64, f32) {
        ...
        (self.keys.len(), self.retrieval_count, self.successful_recalls, recall_rate)
    }
...
mod tests {
    ...
    fn test_random_vectors_orthogonal() {
        ...
        assert!(sim > 0.4 && sim < 0.6, "Random vectors should be ~orthogonal, got {}", sim);
    }
    ...
    fn test_bind_dissimilarity() {
        ...
        assert!(sim_ac > 0.4 && sim_ac < 0.6, "Bound should be dissimilar to A");
        assert!(sim_bc > 0.4 && sim_bc < 0.6, "Bound should be dissimilar to B");
    }
    ...
    fn test_feature_encoding() {
        ...
        assert!(sim_12 > sim_13, "Similar features should encode similarly: {} vs {}", sim_12, sim_13);
    }
}
