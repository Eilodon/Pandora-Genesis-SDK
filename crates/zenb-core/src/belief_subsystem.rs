...
use crate::belief::{
    BeliefEngine, BeliefState, BeliefBasis, FepState,
    Context, SensorFeatures, PhysioState, softmax, hysteresis_collapse,
};
...
impl BeliefSubsystem {
    ...
    pub fn update(
        ...
        let smoothed: [f32; 5] = std::array::from_fn(|i| {
            self.state.p[i] * (1.0 - alpha) + probs[i] * alpha
        });
        ...
        Ok(BeliefUpdateResult {
            ...
            transitioned,
            previous_mode: if transitioned { Some(previous_mode) } else { None },
            fe_delta,
        })
    }
...
mod tests {
    ...
    fn test_hysteresis() {
        ...
        let features = SensorFeatures {
            hr_bpm: Some(70.0), rmssd: Some(45.0), rr_bpm: Some(12.0), quality: 0.9, motion: 0.1,
        };
        let phys = PhysioState { 
            hr_bpm: Some(70.0), rr_bpm: Some(12.0), rmssd: Some(45.0), confidence: 0.9,
        };
        let ctx = Context { local_hour: 14, is_charging: false, recent_sessions: 5 };
