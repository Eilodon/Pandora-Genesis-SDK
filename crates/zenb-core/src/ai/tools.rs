...
impl AiTool for AdjustTempoTool {
    ...
    fn can_execute(&self, args: &Value, ctx: &ToolContext) -> Result<(), String> {
        let scale = args["scale"].as_f64().ok_or("Missing 'scale' parameter")? as f32;
    ...
    fn execute(&self, args: &Value, _ctx: &ToolContext) -> Result<String, String> {
        ...
        let reason = args["reason"].as_str().unwrap_or("AI adjustment");
...
impl AiTool for SwitchPatternTool {
    ...
    fn can_execute(&self, args: &Value, ctx: &ToolContext) -> Result<(), String> {
        let pattern_id = args["pattern_id"].as_str().ok_or("Missing 'pattern_id'")?;
...
impl AiToolRegistry {
    ...
    pub fn execute(&self, tool_name: &str, args: &Value, ctx: &ToolContext) -> ToolResult {
