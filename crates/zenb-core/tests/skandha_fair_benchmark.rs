...
fn test_sheaf_consensus_benefit() {
    ...
    let baseline_belief = baseline_engine.skandha_pipeline.vedana.state().clone();
    ...
    let vajra_belief = vajra_engine.skandha_pipeline.vedana.state().clone();
...
fn test_convergence_speed() {
    ...
    for (i, obs) in observations.iter().enumerate() {
        ...
        if baseline_converged_at.is_none()
            && dominant_mode(&baseline_engine.skandha_pipeline.vedana.probabilities()) == 0  // Calm
            && baseline_engine.skandha_pipeline.vedana.probabilities()[0] > 0.5
        {
    ...
    for (i, obs) in observations.iter().enumerate() {
        ...
        if vajra_converged_at.is_none()
            && dominant_mode(&vajra_engine.skandha_pipeline.vedana.probabilities()) == 0
            && vajra_engine.skandha_pipeline.vedana.probabilities()[0] > 0.5
        {
...
fn test_pattern_memory_benefit() {
    ...
    let baseline_stress_conf = baseline_engine.skandha_pipeline.vedana.probabilities()[1]; // Stress mode
    ...
    let vajra_stress_conf = vajra_engine.skandha_pipeline.vedana.probabilities()[1];
