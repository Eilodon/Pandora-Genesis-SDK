...
fn test_efe_replaces_heuristic_selection() {
    ...
    assert!(engine.last_selected_policy.is_some(), "EFE selection should populate last_selected_policy");
    ...
    assert!(engine.efe_meta_learner.success_rate_ema > 0.5, "Success rate should increase");
}
