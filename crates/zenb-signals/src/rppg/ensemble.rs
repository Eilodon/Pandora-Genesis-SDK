...
impl EnsembleProcessor {
    ...
    pub fn process(&mut self) -> Option<EnsembleResult> {
        ...
        if self.config.use_pos {
            if let Some((bpm, confidence)) = self.pos_processor.process() {
                let snr = self.confidence_to_snr(confidence);
                if snr >= self.config.min_snr_threshold {
        ...
        if self.config.use_chrom {
            if let Some((bpm, confidence)) = self.chrom_processor.process() {
                let snr = self.confidence_to_snr(confidence);
                if snr >= self.config.min_snr_threshold {
                    results.push(AlgorithmResult {
                        bpm,
    ...
    pub fn process_arrays(
        ...
        // Prepare buffer-based processors
        self.reset();
        for i in 0..r.len() {
            self.add_sample(r[i], g[i], b[i]);
        }
    ...
    /// Combine results using SNR-weighted voting
    fn combine_results(
        &self,
        ...
        // Average confidence
        let avg_confidence: f32 = results.iter().map(|r| r.confidence).sum::<f32>()
            / results.len() as f32;

        // Average SNR
        let avg_snr: f32 = results.iter().map(|r| r.snr).sum::<f32>() / results.len() as f32;
    ...
    /// Convert confidence (0-1) back to approximate SNR
    fn confidence_to_snr(&self, confidence: f32) -> f32 {
        // Reverse of SNR-to-confidence mapping
        confidence * 15.0 - 5.0
    }
