...
impl RppgProcessor {
    ...
    pub fn new(method: RppgMethod, window_size: usize, sample_rate: f32) -> Self {
        Self {
            ...
            window_size,
            sample_rate,
            buffer_r: VecDeque::with_capacity(window_size * 2),
            buffer_g: VecDeque::with_capacity(window_size * 2),
            buffer_b: VecDeque::with_capacity(window_size * 2),
        }

    /// Convert the tail of a ring buffer into an `Array1` without O(n) front-removals.
    #[inline]
    fn tail_to_array1(buf: &VecDeque<f32>, window_size: usize) -> Array1<f32> {
        let len = buf.len();
        let start = len.saturating_sub(window_size);
        Array1::from_iter(buf.iter().skip(start).cloned())
    }

    /// Process and return full metrics (HR, confidence, SNR, and pulse waveform).
    ///
    /// This keeps the legacy `process()` API available while exposing the richer `RppgResult`.
    pub fn process_result(&self) -> Option<RppgResult> {
        if !self.is_ready() {
            return None;
        }

        let r = Self::tail_to_array1(&self.buffer_r, self.window_size);
        let g = Self::tail_to_array1(&self.buffer_g, self.window_size);
        let b = Self::tail_to_array1(&self.buffer_b, self.window_size);

        self.process_window_result(&r, &g, &b)
    }

    /// Stateless processing over a provided RGB window.
    ///
    /// Useful for batch/offline evaluation and for ensemble processing without re-buffering.
    pub fn process_window_result(
        &self,
        r: &Array1<f32>,
        g: &Array1<f32>,
        b: &Array1<f32>,
        ...
        // Compute heart rate via FFT (bpm + snr)
        let (bpm, snr) = DspProcessor::compute_heart_rate(&filtered, self.sample_rate);

        // Map SNR to confidence using a smooth logistic curve (more stable than linear clamping)
        // - Below ~0 dB confidence collapses quickly
        // - Around ~5 dB ramps strongly
        // - Above ~10 dB saturates near 1.0
        let confidence = 1.0 / (1.0 + (-0.6 * (snr - 5.0)).exp());

        Some(RppgResult {
            heart_rate: bpm,
            confidence,
            snr,
            pulse_waveform: filtered.to_vec(),
        })
    }
    }
    ...
    pub fn add_sample(&mut self, r: f32, g: f32, b: f32) {
        ...
        // Keep 2x window for overlap (efficient O(1) pop_front)
        let max_samples = self.window_size * 2;
        while self.buffer_r.len() > max_samples {
            let _ = self.buffer_r.pop_front();
            let _ = self.buffer_g.pop_front();
            let _ = self.buffer_b.pop_front();
        }
    }
    ...
    pub fn process(&self) -> Option<(f32, f32)> {
        self.process_result().map(|r| (r.heart_rate, r.confidence))
    }
